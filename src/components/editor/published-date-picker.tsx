...
function parseDate(dateString: string): Date {
  ...
  if (parts.length === 3) {
    ...
    if (monthIndex !== -1) {
      const d = new Date(
        Date.UTC(parseInt(parts[2]), monthIndex, parseInt(parts[0])),
      );
...
export function PublishedDatePicker({
  ...
  const handleSelect = (day: number) => {
    const newDate = new Date(
      Date.UTC(selectedDate.getUTCFullYear(), selectedDate.getUTCMonth(), day),
    );
        ...
        <div className="absolute left-0 top-[calc(100%+0.5rem)] z-20 w-full max-h-80 overflow-y-auto rounded-[1.6rem] border border-white/10 bg-[#0c0c0c]/96 p-3 backdrop-blur-xl">
          <div className="flex items-center justify-between mb-3">
            <button
              ...
              className="p-1 text-white/42 hover:text-white rounded-full p-2"
            >
              <ChevronLeft className="size-4" />
            </button>
            <span className="text-sm font-medium text-white">
              {MONTHS[selectedDate.getUTCMonth()]}{" "}
              {selectedDate.getUTCFullYear()}
            </span>
            <button
              type="button"
          ...
          <div className="grid grid-cols-7 mb-1">
            {["S", "M", "T", "W", "Th", "F", "Sa"].map((d) => (
              <span
                key={d}
                className="text-center text-[0.6rem] uppercase text-white/36 py-1"
              >
                {d}
              </span>
            ))}
